...
class Product(ProductBase):
    ...
    merchant_id: str
    cost_price: float
    baseline_price: float          # merchant-set anchor; defaults to price at creation
...
class BusinessConstraints(BaseModel):
    ...
    # Merchant-set headroom above baseline_price a live price is allowed to
    # rise to. Default 0.0 means "no uplift allowed" — an existing store that
    # hasn't opted in stays discount-only (today's behavior) until the
    # merchant explicitly sets this above zero.
    max_uplift_percent: float = Field(ge=0, le=100, default=0.0)
