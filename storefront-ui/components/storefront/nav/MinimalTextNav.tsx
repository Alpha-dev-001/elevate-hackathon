...
export function MinimalTextNav({ store, activeCategory, onSelect }: NavProps) {
  ...
  return (
    <nav
      className="nav-links flex items-center gap-x-3 gap-y-0 overflow-x-auto whitespace-nowrap px-5 md:px-10 py-4 text-sm
    ...
    >
      {items.map((c, i) => (
        <Fragment key={c ?? 'all'}>
          ...
          <button
            onClick={() => onSelect(c)}
            className="nav-link shrink-0 transition-colors hover:opacity-70"
            style={{
